...
def run_training_scenario(scenario: int):
    if scenario < 3:
        run_training(
            local_store.PATHS[local_store.MODEL_ARTIFACTS],
            model_info.SUFFIX,
            model_info.FILENAMES[scenario],
            model_info.MODEL_INFO[scenario][model_info.LAST_PERCENTAGE],
            os.path.join(local_store.PATHS[local_store.ENEX_BATCH_CSV], "summary.csv"),
            os.path.join(local_store.PATHS[local_store.INTERNET_SINGLE_CSV], "summary.csv")
        )
    else:
        raise ValueError(f"Unknown scenario: {scenario}")

def run_training(mpath: str,
                 suffix: str,
                 filenames: dict[str, str],
                 last_percentage: int,
                 csv_path: str,
                 int_path: str | None = None) -> dict:
    """
    Startet das Training mit den angegebenen Parametern.
    Gibt ein kleines Summary-Dict zurück (kannst du nach Bedarf erweitern).
    """

    # die 4 "globalen" Variablen lokal setzen, damit der alte Code fast unverändert bleiben kann
    SUFFIX = suffix
    MODEL_PATH = mpath
    CSV_PATH = csv_path
    INT_PATH = int_path
    ...
    sys.stdout.write(
        ...
        + ", csv_path=" + CSV_PATH
        + ", int_path=" + str(INT_PATH)
        + ", last_percentage=" + str(LAST_PERCENTAGE) + "\n"
    )
    sys.stdout.flush()
    ...
    if INT_PATH is not None and not Path(INT_PATH).exists():
        INT_PATH = None

    # Szenario "alles" ohne INT_PATH
    ...
    joblib.dump(pipe,       os.path.join(MODEL_PATH, filenames[model_info.FILENAME_PIPELINE]))
    joblib.dump(mlb,        os.path.join(MODEL_PATH, filenames[model_info.FILENAME_LABELS]))
    joblib.dump(thresholds, os.path.join(MODEL_PATH, filenames[model_info.FILENAME_THRESHOLDS]))
    report_path = os.path.join(MODEL_PATH, filenames[model_info.FILENAME_REPORT])
    ...
    result = {
        ...
        "labels_total": int(len(mlb.classes_)),
        "artifacts": {
            "pipeline":   os.path.abspath(os.path.join(MODEL_PATH, filenames[model_info.FILENAME_PIPELINE])),
            "labels":     os.path.abspath(os.path.join(MODEL_PATH, filenames[model_info.FILENAME_LABELS])),
            "thresholds": os.path.abspath(os.path.join(MODEL_PATH, filenames[model_info.FILENAME_THRESHOLDS])),
            "report":     os.path.abspath(os.path.join(MODEL_PATH, filenames[model_info.FILENAME_REPORT])),
        }
    }

    print("ended training " + datetime.now().strftime("%H:%M:%S"))
    print("results written to " + MODEL_PATH)
    return result
...
if __name__ == "__main__":
    ...
    if (args.scenario is None):
        run_training(
            ...
            suffix=args.suffix,
            filenames={
                "pipeline": f"pipeline_{args.suffix}.joblib",
                "labels": f"labels_{args.suffix}.joblib",
                "thresholds": f"thresholds_{args.suffix}.joblib",
                "report": f"report_{args.suffix}.txt"
            },
        ...
        run_training_scenario(args.scenario)